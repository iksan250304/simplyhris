...
function AdminLogoMark() {
  return (
    <div className="flex-shrink-0 flex items-center justify-center" style={{ width: 54, height: 30 }}>
...
export default function AdminNavbar() {
  ...
  return (
    <nav
      className="sticky top-0 z-50 border-b flex items-center justify-between px-6 py-0"
      style={{ background: "#ffffff", borderColor: "#dde8f0", height: 60, boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}
    >
      <button onClick={() => router.push("/admin/dashboard")} className="flex items-center gap-3">
        <AdminLogoMark />
        <div className="flex items-center gap-2">
          <span className="font-semibold text-[15px]" style={{ color: "#1a2a3a" }}>Simply HRIS</span>
          <span className="text-xs font-medium px-2 py-0.5 rounded-full" style={{ background: "rgba(11,137,196,0.12)", color: "#0873a8" }}>
            Admin
          </span>
        </div>
      </button>
      ...
      <div className="flex items-center gap-1">
        {links.map((l) => (
          <button
            ...
            style={{
              background: pathname === l.href ? "rgba(11,137,196,0.1)" : "transparent",
              color: pathname === l.href ? "#0B89C4" : "#6a8faa",
              border: pathname === l.href ? "1px solid rgba(11,137,196,0.25)" : "1px solid transparent",
            }}
          >
            {l.label}
          </button>
        ...
        <button
          onClick={logout}
          className="ml-3 text-sm font-medium px-4 py-1.5 rounded-lg transition-all"
          style={{ background: "#f0f4f8", color: "#6a8faa", border: "1px solid #dde8f0" }}
        >
          Logout
        </button>
      </div>
